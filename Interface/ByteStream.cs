...
namespace XDay
{
    public sealed class ByteStream : Stream
    {
        ...
        public void Init()
        {
            if (m_Bytes != null)
            {
                Log.Instance?.Error($"stream already inited");
                return;
            }
            m_Position = 0;
            m_Length = 0;
                ...
                case SeekOrigin.Begin:
                    {
                        ...
                        {
                            Log.Instance?.Error($"Invalid seek offset {offset}");
                        }
                        break;
                    }
                case SeekOrigin.End:
        ...
        {
            Debug.Assert(n > 0);
            var availableSize = Math.Clamp(m_Length - m_Position, 0, n);
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            var readCount = Math.Clamp(m_Length - m_Position, 0, count);
            if (readCount > 8)
            {
