    ...
    public sealed class PhysicsWorld
    {
        ...
        private void BroadPhase()
        {
            for (int i = 0; i < m_BodyList.Count - 1; i++)
            {
                ...
                FixedAABB bodyA_aabb = bodyA.GetAABB();

                for (int j = i + 1; j < m_BodyList.Count; j++)
                {
                    Rigidbody bodyB = m_BodyList[j];
                    FixedAABB bodyB_aabb = bodyB.GetAABB();
                    ...
                    if (!FixedCollision.IntersectAABBs(bodyA_aabb, bodyB_aabb))
                    {
                        continue;
                    }
        ...
        private void NarrowPhase()
        {
            foreach (var pair in m_ContactPairs)
            {
                ...
                if (FixedCollision.Collide(bodyA, bodyB, out FixedVector2 normal, out FixedPoint depth))
                {
                    ...
                    FixedManifold contact = new FixedManifold(bodyA, bodyB, normal, depth, contact1, contact2, contactCount);
